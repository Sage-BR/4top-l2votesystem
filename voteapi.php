...
function buildHandler($top, $token, $serverId) {
    ...
    static $map = array(
        ...
        '4top'        => 'FourTopTop',
        'hopzone'     => 'HopzoneHRATop',
        'hopzoneu'    => 'HopzoneTop',
        'itopz'       => 'ITopZTop',
        'l2toporg'    => 'L2TopOrgTop',
        'hotservers'  => 'HotServersTop',
        'l2rankzone'  => 'L2RankZoneTop',
    );
...
// Hopzone.eu
// =============================================================================
class HopzoneTop extends TopBase {
    protected $name        = 'Hopzone.eu';
    protected $apiTimezone = 'UTC';
    const API_URL          = 'https://api.hopzone.eu/v1/';
    const VOTE_URL         = 'https://hopzone.eu/vote/';

    public function checkVote($ip, $login = '') {
        if (empty($this->token))             return TopResult::fail('Hopzone.eu: API Key não configurada');
        if (empty($ip) || $ip === 'UNKNOWN') return TopResult::fail('Hopzone.eu: IP inválido');

        $url  = self::API_URL . '?api_key=' . urlencode($this->token) . '&ip=' . urlencode($ip) . '&type=json';
        $body = $this->httpGet($url);
        if (!$body) return TopResult::fail('Hopzone.eu API inacessível');

        $data = $this->decodeJson($body);
        if (!is_array($data)) return TopResult::fail('Hopzone.eu: resposta inválida');

        if (($data['error'] ?? '') !== 'OK')
            return TopResult::fail('Hopzone.eu: ' . ($data['error'] ?? 'UNKNOWN'));

        $status   = $data['status']               ?? '';
        $hours    = (float)($data['hours_since_vote'] ?? 99);
        $voteTime = (int)($data['vote_time']           ?? 0);

        // vote_time já é Unix UTC nesta API — sem conversão necessária
        $this->log("status=$status | hours=$hours | voteTime=$voteTime | ip=$ip");

        return ($status === 'completed' && $hours < 24)
            ? TopResult::ok($voteTime, $data)
            : TopResult::notVoted("status=$status hours=$hours");
    }

    public function getVoteUrl($login = '') {
        $url  = self::API_URL . '?api_key=' . urlencode($this->token) . '&generate=' . urlencode($this->serverId) . '&type=json';
        $body = $this->httpGet($url);
        if ($body) {
            $data = $this->decodeJson($body);
            if (!empty($data['url'])) return $data['url'];
        }
        return self::VOTE_URL . urlencode($this->serverId);
    }
}
...
// Hopzone.net
// =============================================================================
class HopzoneHRATop extends TopBase {
    protected $name        = 'Hopzone.net';
    protected $apiTimezone = 'UTC';
    const API_URL          = 'https://api.hopzone.net/lineage2/vote';
    const VOTE_URL         = 'https://l2.hopzone.net/site/vote/';
    const VOTE_WINDOW      = 43200;

    public function checkVote($ip, $login = '') {
        if (empty($this->token))             return TopResult::fail('Hopzone.net: API Key não configurada');
        if (empty($this->serverId))          return TopResult::fail('Hopzone.net: Server ID não configurado');
        if (empty($ip) || $ip === 'UNKNOWN') return TopResult::fail('Hopzone.net: IP inválido');

        $url  = self::API_URL . '?' . http_build_query(array('token' => $this->token, 'ip_address' => $ip));
        $body = $this->httpGet($url);
        if (!$body) return TopResult::fail('Hopzone.net API inacessível');

        $data = $this->decodeJson($body);
        if (!is_array($data) || !array_key_exists('voted', $data))
            return TopResult::fail('Hopzone.net: resposta inválida');

        $voted = filter_var($data['voted'], FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? !empty($data['voted']);

        // voteTime pode ser Unix timestamp ou string de data
        $rawVoteTime = $data['voteTime'] ?? null;
        $voteTs = 0;
        if ($rawVoteTime && $rawVoteTime !== 'n/a') {
            $voteTs = is_numeric($rawVoteTime)
                ? (int)$rawVoteTime
                : $this->parseDateToUtc((string)$rawVoteTime);
        }

        $this->log("voted=$voted | voteTs=$voteTs | ip=$ip");

        if (!$voted || !$this->isVoteValid($voteTs, self::VOTE_WINDOW)) {
            return TopResult::notVoted('Não votou');
        }

        return TopResult::ok($voteTs, array('voted' => $data['voted'], 'voteTime' => $rawVoteTime));
    }

    public function getVoteUrl($login = '') {
        return self::VOTE_URL . urlencode($this->serverId) . '/1';
    }
}
...
// iTopZ
// =============================================================================
class ITopZTop extends TopBase {
    protected $name        = 'iTopZ';
    protected $apiTimezone = 'UTC';
    const API_BASE         = 'https://itopz.com/check';
    const VOTE_BASE        = 'https://itopz.com/vote';
    const VOTE_WINDOW      = 43200;

    public function checkVote($ip, $login = '') {
        if (empty($this->token))             return TopResult::fail('iTopZ: API Key não configurada');
        if (empty($this->serverId))          return TopResult::fail('iTopZ: Server ID não configurado');
        if (empty($ip) || $ip === 'UNKNOWN') return TopResult::fail('iTopZ: IP inválido');

        $url  = self::API_BASE . '/' . urlencode($this->token) . '/' . urlencode($this->serverId) . '/' . urlencode($ip);
        $body = $this->httpGet($url);
        if (!$body) return TopResult::fail('iTopZ API inacessível');

        $data = $this->decodeJson($body);
        if (!$data || !isset($data['voteTime'])) return TopResult::fail('iTopZ: resposta inválida');

        // iTopZ retorna Unix timestamp UTC diretamente
        $voteTime = (int)$data['voteTime'];
        $this->log("voteTime=$voteTime | ip=$ip");

        if ($voteTime <= 0)                              return TopResult::notVoted('Não votou');
        if ($this->isVoteValid($voteTime, self::VOTE_WINDOW)) return TopResult::ok($voteTime);
        return TopResult::notVoted('Voto expirado');
    }

    public function getVoteUrl($login = '') {
        return self::VOTE_BASE . '/' . urlencode($this->serverId);
    }
}
...
// HotServers.org
// =============================================================================
class HotServersTop extends TopBase {
    protected $name        = 'HotServers';
    protected $apiTimezone = 'UTC';
    const API_URL          = 'https://www.hotservers.org/api/servers/';
    const VOTE_URL         = 'https://www.hotservers.org/servers/';
    const VOTE_WINDOW      = 43200;

    public function checkVote($ip, $login = '') {
        if (empty($this->token))             return TopResult::fail('HotServers: API Token não configurado');
        if (empty($this->serverId))          return TopResult::fail('HotServers: Server ID não configurado');
        if (empty($ip) || $ip === 'UNKNOWN') return TopResult::fail('HotServers: IP inválido');

        $url  = self::API_URL . urlencode($this->serverId) . '/vote/check/' . urlencode($ip) . '?api_token=' . urlencode($this->token);
        $body = $this->httpGet($url);
        if (!$body) return TopResult::fail('HotServers API inacessível');

        $data = $this->decodeJson($body);
        if (!$data) return TopResult::fail('HotServers: resposta inválida');

        if (!empty($data['response_error']) && $data['response_error'] !== false)
            return TopResult::fail('HotServers: ' . $data['response_error']);

        $hasVoted = (bool)($data['has_voted'] ?? false);
        $voteTime = (int)($data['vote_time']  ?? 0);

        $this->log("has_voted=$hasVoted | voteTime=$voteTime | ip=$ip");

        return ($hasVoted && $this->isVoteValid($voteTime, self::VOTE_WINDOW))
            ? TopResult::ok($voteTime, $data)
            : TopResult::notVoted('Não votou');
    }

    public function getVoteUrl($login = '') {
        return self::VOTE_URL . urlencode($this->serverId) . '/vote';
    }
}
...
// L2RankZone
// =============================================================================
class L2RankZoneTop extends TopBase {
    protected $name        = 'L2RankZone';
    protected $apiTimezone = 'UTC';
    const API_URL          = 'https://l2rankzone.com/api/vote-reward';
    const VOTE_WINDOW      = 43200;

    public function checkVote($ip, $login = '') {
        if (empty($this->token))             return TopResult::fail('L2RankZone: API Key não configurada');
        if (empty($ip) || $ip === 'UNKNOWN') return TopResult::fail('L2RankZone: IP inválido');

        $url  = self::API_URL . '?apiKey=' . urlencode($this->token) . '&ip=' . urlencode($ip);
        $body = $this->httpGet($url);
        if (!$body) return TopResult::fail('L2RankZone API inacessível');

        $data = $this->decodeJson($body);
        if (!$data || !isset($data['voted'])) return TopResult::fail('L2RankZone: resposta inválida');

        $voted    = (bool)$data['voted'];
        $voteTime = (int)($data['voteTime'] ?? 0);

        $this->log("voted=$voted | voteTime=$voteTime | ip=$ip");

        return ($voted && $this->isVoteValid($voteTime, self::VOTE_WINDOW))
            ? TopResult::ok($voteTime, $data)
            : TopResult::notVoted('Não votou');
    }

    public function getVoteUrl($login = '') {
        $parts = explode('/', $this->serverId, 2);
        $path  = implode('/', array_map('rawurlencode', $parts));
        return 'https://l2rankzone.com/lineage2-servers/' . $path . '/vote';
    }
}
